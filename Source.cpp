...
class Arry_chang {
public:
	void arry_chang(double *enter_data, double *box, int count);
};

void Arry_chang::arry_chang(double *enter_data, double *box, int count) {
	printf("%d", count);
...
void optimize(struct particle ps[])
{
	...
	for (i = 0; i<NOPS; i++) {
		...
		if (ps[i].value<ps[i].bestval) {
			...
			cg.arry_chang(ps[i].pos, ps[i].bestpos,DIM);
/*			for (int j = 0; j < DIM; j++) {
